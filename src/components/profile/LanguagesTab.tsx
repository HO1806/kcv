...
export function LanguagesTab({ profile, onSave }: LanguagesTabProps) {
  const [items, setItems] = useState<Language[]>(profile.languages)
  const [editing, setEditing] = useState<Language | null | 'new'>()

  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => setItems(profile.languages), [profile.languages])
  useAutoSave(items, (v) => onSave({ languages: v }))
  ...
  function handleSave(item: Language) {
    setItems((prev) => {
      ...
      {items.length === 0 ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>Ajoutez les langues que vous parlez.</EmptyDescription>
          </EmptyHeader>
        </Empty>
      ) : (
...
function SortableLanguageCard({ lang, onEdit, onDelete }: SortableLanguageCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="ks-body-sm font-semibold truncate">
          {lang.name || <span className="text-muted-foreground">Langue non renseignée</span>}
        </span>
